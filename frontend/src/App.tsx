...
import { prepareTranscriptImages, selectTranscriptImages } from './lib/image-import';
import {
  applyOcrConfidence, confirmImportableCandidates, deduplicateOcrCandidates,
  isImportableOcrCandidate, parseTranscriptOcrText,
  UI_EXAM_TYPES, UI_LETTER_GRADES, UI_PASS_FAIL_GRADES,
...
function ImportPage({ onImport }: { onImport: (records: CourseAttempt[]) => void }) {
  ...
  const parseText = () => {
    const parsed = deduplicateOcrCandidates(parseTranscriptOcrText(rawText, semester));
    setCandidates(parsed.unique);
    setOcrStatus(parsed.duplicateCount
      ? `已解析 ${parsed.unique.length} 条，并删除 ${parsed.duplicateCount} 条重复课程记录。`
      : `已解析 ${parsed.unique.length} 条记录，请核对后导入。`);
  };
  const recogniseAndParseImages = async (files: File[], rejectedCount = 0, duplicateImageCount = 0) => {
    ...
    try {
      ...
      const parsed = deduplicateOcrCandidates(parseTranscriptOcrText(combinedText, semester));
      setRawText(combinedText); setCandidates(applyOcrConfidence(parsed.unique, averageConfidence));
      const rejectedMessage = rejectedCount ? `，已忽略 ${rejectedCount} 个不支持的文件` : '';
      const duplicateImageMessage = duplicateImageCount ? `，已删除 ${duplicateImageCount} 张重复截图` : '';
      const duplicateRowMessage = parsed.duplicateCount ? `，并删除 ${parsed.duplicateCount} 条重复课程记录` : '';
      setOcrStatus(`识别完成，共 ${results.length} 张${duplicateImageMessage}${rejectedMessage}${duplicateRowMessage}，平均文本置信度 ${averageConfidence.toFixed(0)}%。请核对后导入。`);
  ...
  const receiveFiles = async (files: Iterable<File>) => {
    ...
    setBusy(true); setOcrStatus('正在检查图片格式和重复内容…');
    try {
      ...
      if (prepared.unique.length === 0) {
        setOcrStatus(prepared.duplicateCount
          ? `选择的图片均为重复截图，已自动删除 ${prepared.duplicateCount} 张。`
          : '没有通过格式检查的图片，请重新截图后上传。');
        setBusy(false);
        return;
      }
      await recogniseAndParseImages(prepared.unique, rejectedCount, prepared.duplicateCount);
    } catch (error) {
      setBusy(false);
      setOcrStatus(error instanceof Error ? `图片检查失败：${error.message}` : '图片检查失败，请重新选择。');
    }
  };
  const update = (id: string, key: keyof OcrCandidate, value: string) => setCandidates((items) => items.map((item) => {
    if (item.id !== id) return item;
    if (key === 'credits') return { ...item, credits: value.trim() === '' ? -1 : Number(value), needsReview: true, confirmed: false };
    ...
    if (key === 'grade' && value.trim() === '') { const { grade: _removed, ...rest } = item; return { ...rest, needsReview: true, confirmed: false }; }
    return { ...item, [key]: value, needsReview: true, confirmed: false };
  }));
  const importRecords = () => {
    const invalid = candidates.some((item) => !isImportableOcrCandidate(item) || !item.confirmed);
...
function SimulatorPage({ attempts, summary }: { attempts: CourseAttempt[]; summary: GpaSummary }) {
  ...
  useEffect(() => { let ignore = false; const future = courses.map((course) => ({ id: course.id, courseName: course.name || '未命名规划课程', semester: '规划学期', credits: course.credits.trim() === '' ? 0 : Number(course.credits), grade: course.grade, examType: 'NORMAL' as UiExamType })); void calculateGpa([...attempts, ...future]).then((result) => { if (!ignore) { setProjected(result); setSimulationError(''); } }).catch((error: unknown) => { if (!ignore) { setProjected(null); setSimulationError(error instanceof Error ? error.message : '模拟课程数据无法计算。'); } }); return () => { ignore = true; }; }, [attempts, courses]);
  ...
  return <><header className="page-header"><div><p className="eyebrow">未来情景</p><h1>学期模拟器</h1><p>模拟成绩不会写入正式成绩档案。</p></div></header>{simulationError && <div className="notice warning">{simulationError}</div>}<section className="two-column"><div className="panel"><div className="panel-title"><h2>规划课程</h2><button className="secondary" onClick={add}><Plus size={16} />添加课程</button></div><div className="simulation-list">{courses.map((course) => <div className="simulation-row" key={course.id}><input aria-label={`${course.name} 课程名称`} value={course.name} onChange={(e) => update(course.id, 'name', e.target.value)} /><input aria-label={`${course.name} 学分`} type="number" min="0" max="100" value={course.credits} onChange={(e) => update(course.id, 'credits', e.target.value)} /><select aria-label={`${course.name} 目标等级`} value={course.grade} onChange={(e) => update(course.id, 'grade', e.target.value)}>{UI_LETTER_GRADES.map((grade) => <option value={grade} key={grade}>{gradeLabels[grade]}</option>)}</select><button aria-label={`删除规划课程 ${course.name}`} className="icon-button" onClick={() => setCourses((items) => items.filter((item) => item.id !== course.id))}><Trash2 size={16} /></button></div>)}</div></div><div className="panel simulation-summary"><p className="eyebrow">预测结果</p><div className="gpa-number dark">{futureCredits ? (futurePoints / futureCredits).toFixed(2) : '—'}</div><p>本学期预计 GPA</p><div className="compare"><span>当前累计 GPA <strong>{summary.displayGpa ?? '—'}</strong></span><ChevronRight size={16} /><span>预计累计 GPA <strong>{projected?.displayGpa ?? '—'}</strong></span></div><p className="muted">假设未来课程全部计入 GPA，且不影响既有考试记录。</p></div></section></>;
}
...
function CourseScorePage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">单科反推</p><h1>剩余考核需要多少分？</h1><p>请保留一个未知成绩项，且所有权重合计必须为 100%。</p></div></header><section className="panel"><div className="course-score-layout"><div><label>目标总评<input type="number" min="0" max="100" value={targetScore} onChange={(e) => setTargetScore(e.target.value)} /></label><div className="component-list">{components.map((component, index) => <div className="component-row" key={index}><input aria-label={`考核项 ${index + 1} 名称`} value={component.name} onChange={(e) => setComponents((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, name: e.target.value } : item))} /><input aria-label={`${component.name || `考核项 ${index + 1}`} 权重`} type="number" min="0.01" max="100" step="0.01" value={component.weight} onChange={(e) => setComponents((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, weight: e.target.value } : item))} /><input aria-label={`${component.name || `考核项 ${index + 1}`} 成绩`} type="number" min="0" max="100" step="0.01" placeholder="未知" value={component.score} onChange={(e) => setComponents((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, score: e.target.value } : item))} /><button aria-label={`删除考核项 ${component.name || index + 1}`} className="icon-button" onClick={() => setComponents((items) => items.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={16} /></button></div>)}</div><div className="header-actions"><button className="secondary" onClick={() => setComponents((items) => [...items, { name: '新考核项', weight: '10', score: '' }])}><Plus size={16} />添加考核项</button><button className="primary" onClick={() => void calculate()}>反推成绩</button></div></div><div className="course-score-result">{error && <div className="notice warning">{error}</div>}{result ? <><StatusPill status={result.status} /><p>在 <strong>{result.unknownComponent}</strong> 中至少需要</p><strong className="plan-number">{result.status === 'ALREADY_ACHIEVED' ? '0.00' : result.displayRequiredScore}</strong><p>已获得加权分：{result.knownWeightedScore.toFixed(2)}</p></> : <p className="muted">输入项目后开始计算。</p>}</div></div></section></>;
}
