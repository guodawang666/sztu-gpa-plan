...
function gpaResponse(attempts: Array<Record<string, unknown>>) {
  const points: Record<string, number> = {
    'A+': 4.5, A: 4, 'B+': 3.5, B: 3, 'C+': 2.5, C: 2, D: 1, F: 0,
  };
  const contributions = attempts.map((attempt) => ({
    ...attempt,
    grade: String(attempt.grade),
    gradePoint: points[String(attempt.grade)] ?? null,
    earnedCredit: attempt.grade === 'F' || attempt.grade === 'NP' ? 0 : Number(attempt.credits),
    includedInGpa: attempt.grade !== 'P' && attempt.grade !== 'NP',
    gpaCredits: attempt.grade === 'P' || attempt.grade === 'NP' ? 0 : Number(attempt.credits),
    qualityPoints: Number(attempt.credits) * (points[String(attempt.grade)] ?? 0),
  }));
  const attemptedCredits = contributions.reduce((sum, item) => sum + Number(item.credits), 0);
  const earnedCredits = contributions.reduce((sum, item) => sum + Number(item.earnedCredit), 0);
...
describe('grade screenshot import flow', () => {
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input, init) => {
      ...
      if (path.endsWith('/api/v1/gpa/calculate')) {
        payload = gpaResponse(body.attempts as Array<Record<string, unknown>>);
      } else if (path.endsWith('/api/v1/gpa/target')) {
        payload = {
          status: 'ACHIEVABLE', currentGpa: 3, targetGpa: 3.3,
          requiredFutureGpa: 3.315, displayRequiredFutureGpa: '3.32',
          maximumReachableGpa: 4.43, displayMaximumReachableGpa: '4.43', futureGpaCredits: 60,
        };
      } else if (path.endsWith('/api/v1/course-score/required')) {
        payload = {
          status: 'ACHIEVABLE', unknownComponent: '期末', knownWeightedScore: 52.8,
          requiredScore: 80.5, displayRequiredScore: '80.50',
        };
      } else {
  ...
  it('drops duplicate screenshots, approves complete rows once, and imports the course', async () => {
    ...
    const importButton = screen.getByRole('button', { name: '确认导入 1 条记录' });
  ...
  it('keeps dashboard, target planning, semester simulation, and course-score planning connected', async () => {
    localStorage.setItem('sztu-gpa-planner-attempts-v1', JSON.stringify({
      version: 1,
      attempts: [{
        id: 'seed', courseName: '测试课程', semester: '2025-2026-1', credits: 3,
        score: 75, grade: 'B', examType: 'NORMAL',
      }],
    }));
    const user = userEvent.setup();
    ...
    await user.click(screen.getByRole('button', { name: '删除考核项 新考核项' }));
